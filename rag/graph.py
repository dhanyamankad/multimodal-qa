...
def stream_agent(messages: list[dict]):
    ...
    stream_mode=["updates", "messages"] is used (rather than just "updates")
    so main.py can ALSO forward real token-by-token chunks of the model's
    final answer as they're generated (a genuine typing effect, not a
    simulated one) in addition to the existing tool-call/tool-result trace
    events. When multiple stream modes are requested, each yielded item is a
    (mode, chunk) tuple; main.py's serializer distinguishes them by mode.
    """
    return AGENT.stream(
        {"messages": messages},
        config={"recursion_limit": RECURSION_LIMIT},
